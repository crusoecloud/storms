...
func Test_NewListSnapshotsCmd(t *testing.T) {
	...
	mockClientProvider := func(context.Context) (storms.StorageManagementServiceClient, io.Closer, error) {
		return &testutil.MockStorMSClient{
			...
			MockGetSnapshots: func(ctx context.Context, in *storms.GetSnapshotsRequest, opts ...grpc.CallOption) (*storms.GetSnapshotsResponse, error) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			cmd := NewListSnapshotsCmd(mockCmdFactory)
			cmd.SetArgs(tt.args)
